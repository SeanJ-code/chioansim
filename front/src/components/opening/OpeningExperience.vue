<template>
  <div v-if="visible" ref="opening" class="opening" role="dialog" aria-label="照安心首頁開場動畫">
    <div ref="artwork" class="opening__artwork">
      <img src="/images/opening/hands-approved.png" width="1536" height="1024" alt="雙手溫柔擡住長者雙手的線稿" />
    </div>
...
async function start() {
  ...
  if (staticDebug) {
    gsap.set([artwork.value, brand.value], { autoAlpha: 1, clipPath: 'inset(0)' });
    return;
  }
  ...
  timeline = gsap.timeline({ defaults: { ease: 'power2.inOut' }, onComplete: finish })
    ...
    .fromTo(artwork.value, { autoAlpha: 1, clipPath: 'inset(0 0 100% 100%)' }, { clipPath: 'inset(0)', duration: 3.8, ease: 'power1.inOut' })
