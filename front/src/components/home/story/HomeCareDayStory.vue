      ...
      <template v-if="scene.id === '05'" #caregiver>
        <section class="scene05-trust" aria-labelledby="scene05-trust-title">
          ...
          <div v-else-if="featuredCaregivers.length" class="trust-carousel" aria-label="已認證居服員">
            <button class="trust-carousel__arrow trust-carousel__arrow--prev" type="button" aria-label="查看上一位居服員" :disabled="carouselIndex === 0" @click="previousCaregiver">
            ...
            <button class="trust-carousel__arrow trust-carousel__arrow--next" type="button" aria-label="查看下一位居服員" :disabled="carouselIndex >= maxCarouselIndex" @click="nextCaregiver">
          ...
          <div v-if="featuredCaregivers.length > 1 && !caregiverLoading && !caregiverErrorMessage" class="trust-carousel__dots" aria-label="居服員輪播位置">
            <button v-for="(_, index) in featuredCaregivers" :key="index" type="button" class="trust-carousel__dot" :class="{ 'is-active': isCaregiverDotActive(index) }" :aria-current="isCaregiverDotActive(index) ? 'true' : undefined" :aria-label="`前往第 ${index + 1} 位居服員`" @click="goToCaregiver(index)"></button>
          </div>
...
const carouselIndex=ref(0);const visibleCaregivers=ref(3);
const featuredCaregivers=computed(()=>props.caregivers.slice(0,7));
const maxCarouselIndex=computed(()=>Math.max(0,featuredCaregivers.value.length-visibleCaregivers.value));
const carouselTrackStyle=computed(()=>({'--visible-count':String(visibleCaregivers.value),transform:`translate3d(-${carouselIndex.value*(100/visibleCaregivers.value)}%,0,0)`}));
function updateVisibleCaregivers(){visibleCaregivers.value=window.innerWidth<600?1:window.innerWidth<1000?2:3;carouselIndex.value=Math.min(carouselIndex.value,maxCarouselIndex.value)}
function previousCaregiver(){carouselIndex.value=Math.max(0,carouselIndex.value-1)}
function nextCaregiver(){carouselIndex.value=Math.min(maxCarouselIndex.value,carouselIndex.value+1)}
function goToCaregiver(index:number){carouselIndex.value=Math.min(index,maxCarouselIndex.value)}
function isCaregiverDotActive(index:number){return index===carouselIndex.value}
...
.scene05-trust{--trust-width:min(1320px,calc(100vw - 96px));position:relative;isolation:isolate;width:var(--trust-width);margin:clamp(300px,42vh,460px) 0 0 calc((100% - var(--trust-width))/2);padding:100px 0 110px;color:#493833}
...
.trust-carousel{position:relative;width:100%}.trust-carousel__viewport{width:100%;overflow:hidden;outline:none}.trust-carousel__viewport:focus-visible{outline:3px solid #eb9079;outline-offset:5px}.trust-carousel__track{--visible-count:3;display:flex;transition:transform .62s cubic-bezier(.22,1,.36,1);will-change:transform}
...
@media(max-width:999px){.scene05-trust{width:calc(100vw - 48px);margin:48px 0 0 calc((100% - (100vw - 48px))/2);padding:72px 0 90px}.scene05-trust::before{top:-110px;height:calc(100% + 110px)}.trust-carousel--loading .trust-carousel__track{--visible-count:2}.trust-carousel .trust-profile:nth-child(n+3){display:flex}.trust-carousel__arrow--prev{left:-20px}.trust-carousel__arrow--next{right:-20px}}
@media(max-width:599px){.scene05-trust{width:100%;margin:30px 0 0;padding:36px 0 20px}.scene05-trust__intro{margin-bottom:26px}.trust-carousel{padding:0 4px}.trust-carousel--loading .trust-carousel__track{--visible-count:1}.trust-carousel .trust-profile{display:flex;flex-basis:100%;min-height:360px;padding:24px 20px;border-inline-width:4px}.trust-carousel__arrow{top:auto;bottom:-67px;width:44px;height:44px}.trust-carousel__arrow--prev{left:calc(50% - 78px)}.trust-carousel__arrow--next{right:calc(50% - 78px)}.trust-carousel__dots{margin-top:8px}.scene05-trust__all{margin-top:54px}}
