    ...
    <div class="needs-scenes" :class="`is-active-${activeIndex + 1}`" aria-label="照護生活場景" @pointerdown="onPointerDown" @pointerup="onPointerUp" @pointercancel="pointerStart = null">
      <article v-for="(slide,index) in slides" :key="slide.id" :ref="el => setSceneRef(el,index)" class="needs-scene" :class="sceneClass(index)" :aria-current="index === activeIndex ? 'true' : undefined" :aria-label="`${slide.id} ${slide.title}`" role="button" tabindex="0" @click="activateScene(index)" @keydown.enter.prevent="activateScene(index)" @keydown.space.prevent="activateScene(index)">
        <div class="needs-scene__asset" role="img" :aria-label="`${slide.title}照護場景`">
          ...
          <template v-if="showAssetDebug"><span>[ASSET REQUIRED]</span><small>{{ slide.asset }}</small></template>
        </div>
...
const slides = [
  { id:'01',title:'備餐',description:['用一餐溫熱的飯菜，','照顧每天的營養與健康。'],asset:'src/assets/home-world/needs/meal.webp',ambient:['steam'] },
  { id:'02',title:'生活陪伴',description:['聊聊天、喝杯茶，','讓日子多一點笑聲。'],asset:'src/assets/home-world/needs/companionship.webp',ambient:['cat','steam'] },
  { id:'03',title:'簡單家務',description:['整理家裡的整潔，','也讓生活更輕鬆舒適。'],asset:'src/assets/home-world/needs/housework.webp',ambient:['cloth','plant'] },
  { id:'04',title:'陪同外出',description:['一起走出家門，','去曬曬太陽，看看更大的世界。'],asset:'src/assets/home-world/needs/outing.webp',ambient:['sunlight','bag'] },
];
...
function activateScene(index:number){
  ...
  const oldCaption=sceneElements[activeIndex.value]?.querySelector<HTMLElement>('.needs-scene__caption');
  const newCaption=sceneElements[index]?.querySelector<HTMLElement>('.needs-scene__caption');
  const reduced=window.matchMedia('(prefers-reduced-motion: reduce)').matches;
  locked.value=true;gsap.killTweensOf([sceneElements,oldCaption,newCaption,activeLabel.value]);
  if(reduced){activeIndex.value=index;locked.value=false;return}
  const timeline=gsap.timeline({defaults:{ease:'care-soft'},onComplete:()=>{gsap.set([sceneElements,oldCaption,newCaption],{clearProps:'all'});locked.value=false}});
  if(oldCaption)timeline.to(oldCaption,{y:-5,opacity:.48,duration:.38},0);
  timeline.to(sceneElements,{x:(_,el)=>Number((el as HTMLElement).dataset.index)<index?-5:5,duration:.28},0).call(()=>{activeIndex.value=index},undefined,.18).to(sceneElements,{x:0,duration:.72},.18);
  if(newCaption)timeline.fromTo(newCaption,{y:6,opacity:.5},{y:0,opacity:1,duration:.5},.2);
  if(activeLabel.value)timeline.fromTo(activeLabel.value,{y:6,opacity:.3},{y:0,opacity:1,duration:.42},.2);
}
...
.needs-scene__asset{position:absolute;inset:0;display:grid;place-content:center;gap:.55rem;color:#876e65;background:radial-gradient(circle at 30% 24%,#fffaf4aa 0 2%,transparent 28%),radial-gradient(circle at 72% 70%,#dfb9a688 0 5%,transparent 35%),linear-gradient(145deg,#f5e9de,#e6cfc1);text-align:center;transition:filter .45s cubic-bezier(.16,1,.3,1)}.needs-scene__asset::after{content:"";position:absolute;inset:0;opacity:.22;background-image:radial-gradient(#8c7066 0.55px,transparent 0.55px);background-size:6px 6px;pointer-events:none}.needs-scene__asset:has(>span)::before{content:"";position:absolute;inset:1.25rem;border:1px dashed #9f7e7066;border-radius:1.5rem}.needs-scene__asset span,.needs-scene__asset small{position:relative;z-index:1}.needs-scene__asset span{font-size:.82rem;font-weight:700;letter-spacing:.12em}.needs-scene__asset small{font-size:.7rem}
...
@media(hover:hover){.needs-scene:not(.is-active):hover{z-index:7;opacity:.76;transform:translateY(-4px) scale(1.01);filter:brightness(1.035)}.needs-scene:not(.is-active):hover .needs-scene__caption{opacity:1}}
...
@media(prefers-reduced-motion:reduce){.needs-scenes,.needs-scene,.needs-scene__asset,.needs-scene__caption,.needs-dots button,.needs-buttons button,.needs-buttons span{transition:none!important}}
</style>
