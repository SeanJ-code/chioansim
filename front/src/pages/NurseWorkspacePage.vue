        ...
        <section class="work-grid">
          ...
          <section class="content-card" aria-live="polite">
            ...
            <div v-else-if="activeSection === 'safety'" class="record-list">
              <button v-for="item in dashboard?.complaints" :key="item._id" type="button" class="safe-report-row" @click="openSafeReport(item)"><span class="safe-severity" :class="`safe-severity--${normalizedSeverity(item.priority).toLowerCase()}`">{{ severityLabel(item.priority) }}</span><div class="record-main"><small>{{ incidentName(item.category) }}</small><h3>{{ item.reportNumber || 'SAFE REPORT' }}</h3><p>{{ safeStatusCopy(item.status) }}・最後更新 {{ dateTime(item.updatedAt || item.createdAt) }}</p></div><span class="tag">{{ complaintStatus(item.status) }}</span><ChevronRight :size="20" /></button>
...
type SafeReport = { _id: string; reportNumber?: string; category: string; description: string; status: string; priority: string; createdAt: string; updatedAt?: string; activities?: Array<{ label: string; createdAt: string }>; replies?: Array<{ _id?: string; authorRole: string; message: string; createdAt: string }> };
