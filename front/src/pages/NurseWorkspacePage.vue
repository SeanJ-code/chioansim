<template>
  <q-page class="nurse-page">
    <main class="nurse-shell">
      <section class="hero">
        <img :src="photoUrl" alt="我的居服員個人近照" />
        <div class="hero-copy">
    ...
    <q-dialog v-model="dialogOpen"><q-card class="form-dialog"><div class="dialog-head"><div><small>{{ currentMenu.eyebrow }}</small><h2>{{ dialogTitle }}</h2><p>{{ dialogCopy }}</p></div><button type="button" aria-label="關閉視窗" @click="dialogOpen = false"><X :size="22" /></button></div>
      ...
      <q-card-actions class="dialog-actions"><q-btn outline no-caps label="先不要" @click="dialogOpen = false" /><q-btn unelevated no-caps :loading="saving" class="primary-btn" :label="dialogType === 'schedule' ? '安排' : dialogType === 'booking' ? '確認承接任務' : dialogType === 'completion' ? '提出完成任務' : '確認送出'" @click="confirmSubmit" /></q-card-actions>
    </q-card></q-dialog>
  </q-page>
</template>
...
type Booking = { _id: string; bookingNumber?: string; scheduledStartAt: string; status: string; attendanceStatus?: string; recipientId?: Named; requesterUserId?: Named; serviceTypeIds?: Named[] };
...
const currentMenu = computed(() => menuItems.find((item) => item.value === activeSection.value)!); const greeting = computed(() => new Date().getHours() < 12 ? '早安' : new Date().getHours() < 18 ? '午安' : '晚安'); const photoUrl = computed(() => { const value = dashboard.value?.profile.profilePhotoUrl; return value ? (value.startsWith('/') ? value : `/${value}`) : '/chioansimicon.svg'; }); const verificationLabel = computed(() => ({ PENDING: '接案資格審核中', APPROVED: '接案資格已通過', REJECTED: '證照需要補件', EXPIRED: '證照已到期' } as Record<string, string>)[dashboard.value?.profile.verificationStatus || 'PENDING']);
...
const actions: Partial<Record<Section, { label: string; type: DialogType }>> = { overview: { label: '修改我的資料', type: 'profile' }, schedule: { label: '安排休假', type: 'schedule' }, journal: { label: '新增工作日誌', type: 'journal' }, safety: { label: '立即安全通報', type: 'incident' }, leave: { label: '提出請假申請', type: 'leave' } }; const sectionAction = computed(() => { const item = actions[activeSection.value]; return item ? { label: item.label, action: () => item.type === 'schedule' ? openSchedule() : openDialog(item.type) } : null; });
const dialogTitle = computed(() => ({ profile: '修改我的資料', schedule: editingAvailabilityId.value ? '修改休假安排' : '安排休假', journal: '寫一篇安心工作日誌', incident: '向管理員提出安全通報', leave: '提出請假申請', review: '留下本次服務評量', booking: '確認這次照護任務', completion: '提出完成任務' }[dialogType.value])); const dialogCopy = computed(() => ({ profile: '姓名與電話會與帳號綁定，避免冒用；接案資格不會因此改變。', schedule: '平日 09:00–17:00 預設開放預約；这里只需要登記休假或暫停服務。', journal: '記錄服務重點、交班提醒或後續需要留意的事情。送出後無法修改。', incident: '若正遭遇立即危險，請先撥打 110 或 119，再留下平台通報。', leave: '送出後由管理員確認；若已有行程，請同步與平台聯繫。', review: '您的回饋只用於改善媒合品質與服務安全。', booking: '請核對使用者、受照護者、服務內容與時間；確認後會立即通知使用者。', completion: '送出後會通知使用者核對；雙方確認後，系統才會正式結案。' }[dialogType.value]));
