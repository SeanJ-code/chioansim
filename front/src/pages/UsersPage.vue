    ...
    <q-dialog v-model="bookingDialog" transition-show="none" transition-hide="none" @show="animateBookingDialogOpen">
      <q-card ref="careScheduleDialogRef" class="care-schedule-dialog">
        ...
        <template v-else>
          ...
          <q-card-section ref="nextSevenDaysRef" class="next-seven-days"><div class="section-divider"><span>接下來 7 天</span></div><div ref="sevenDayRailRef" class="seven-day-list"><button v-for="day in nextSevenDays" :key="day.date" type="button" class="seven-day-row" :class="{ 'has-booking': day.bookings.length, 'is-selected': day.date === selectedBookingDay }" :data-booking-day="day.date" @click="selectedBookingDay = day.date"><span class="seven-day-date"><strong>{{ day.day }}</strong><small>{{ day.month }} 月</small></span><span class="seven-day-copy"><strong>{{ day.relativeLabel }}</strong><span>{{ day.bookings.length ? `${day.bookings.length} 項安排` : '沒有安排' }}</span><small>{{ day.bookings.length ? day.bookings.map(item => `${bookingTime(item)} ${serviceNames(item)}`).join(' ・ ') : '可開始預約' }}</small></span><span class="seven-day-action">{{ day.bookings.length ? '查看' : '預約' }}<ChevronRight :size="18" /></span></button></div></q-card-section>
        </template>
      </q-card>
    </q-dialog>
...
@media(max-width:800px){.care-schedule-dialog{width:calc(100vw - 20px);max-width:calc(100vw - 20px)!important;height:94dvh;border-radius:22px}.care-schedule-heading{padding:18px 18px 12px}.care-schedule-heading h2{font-size:1.5rem}.care-schedule-heading p{display:none}.care-schedule-layout{display:grid;grid-template-columns:1fr;gap:12px;padding:0 14px 12px;overflow-y:auto}.care-calendar-column{width:100%;max-width:340px;justify-self:center;padding:8px}.selected-day-panel{min-height:260px;padding:15px;overflow:visible}.selected-day-heading{align-items:stretch;flex-direction:column}.selected-day-heading :deep(.q-btn){width:100%}.care-booking-card{grid-template-columns:1fr}.care-booking-time{flex-direction:row;align-items:baseline}.care-booking-meta{grid-template-columns:1fr}.care-booking-actions{align-items:stretch;flex-direction:column}.next-seven-days{padding:0 14px 14px}.seven-day-row{width:132px;min-width:132px;min-height:108px}}
