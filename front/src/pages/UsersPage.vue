...
<section class="support-banner" aria-label="需要協助">
  <div class="support-banner__icon">
  ...
  <button
    type="button"
    @click="handleFeatureItem('LINE 專人服務')"
  >
    LINE 專人協助
  </button>
</section>      </template>
    ...
    <q-dialog v-model="bookingDialog" transition-show="none" transition-hide="none" @show="animateBookingDialogOpen">
      <q-card ref="careScheduleDialogRef" class="care-schedule-dialog">
        ...
        <q-scroll-area v-else class="care-schedule-scroll">
          <div class="care-schedule-content">
          <q-card-section class="care-schedule-layout">
            <aside ref="careCalendarRef" class="care-calendar-column">
              ...
              <div class="calendar-legend"><span><i class="legend-dot booked" />有安排</span><span><i class="legend-dot selected" />查看中</span></div>
            </aside>
          ...
          <q-card-section ref="nextSevenDaysRef" class="next-seven-days"><div class="section-divider"><span>接下來 7 天</span></div><div ref="sevenDayRailRef" class="seven-day-list"><button v-for="day in nextSevenDays" :key="day.date" type="button" class="seven-day-row" :class="{ 'has-booking': day.bookings.length, 'is-selected': day.date === selectedBookingDay }" :data-booking-day="day.date" @click="selectBookingDay(day.date)"><span class="seven-day-date"><strong>{{ day.day }}</strong><small>{{ day.month }} 月</small></span><span class="seven-day-copy"><strong>{{ day.relativeLabel }}</strong><span>{{ day.bookings.length ? `${day.bookings.length} 項安排` : '沒有安排' }}</span><small>{{ day.bookings.length ? day.bookings.map(item => `${bookingTime(item)} ${serviceNames(item)}`).join(' ・ ') : '可開始預約' }}</small></span><span class="seven-day-action">{{ day.bookings.length ? '查看' : '預約' }}<ChevronRight :size="18" /></span></button></div></q-card-section>
          </div>
        </q-scroll-area>
      </q-card>
    </q-dialog>

    <q-dialog v-model="orderSummaryDialog" @show="animateOrderSummaryOpen">
      <q-card ref="orderSummaryCardRef" class="order-summary-dialog">
        <q-card-section class="detail-dialog__heading"><div><small>訂單明細與總額</small><h2>{{ orderMonthLabel }}照護支出</h2><p>依目前預約資料彙整，取消的任務不列入。</p></div><button class="detail-dialog__close" type="button" aria-label="關閉訂單明細" v-close-popup><X :size="24" /></button></q-card-section>
        <q-card-section class="order-summary-body">
          <label class="order-month-field"><span>查看月份</span><input v-model="orderMonth" type="month" /></label>
          <section class="order-total" aria-live="polite"><span>本月服務總額</span><strong>NT$ {{ formatMoney(orderMonthTotal) }}</strong><small>{{ orderMonthBookings.length }} 筆服務<span v-if="orderPendingAmountCount">・{{ orderPendingAmountCount }} 筆費用待確認</span></small></section>
          <div v-if="orderMonthBookings.length" class="order-detail-list">
            <article v-for="booking in orderMonthBookings" :key="booking._id" class="order-detail-card"><div><strong>{{ serviceNames(booking) }}</strong><span>{{ formatBookingDate(booking.scheduledStartAt) }}・{{ bookingCaregiverName(booking) }}</span><small>{{ bookingStatusLabel(booking.status) }}</small></div><strong>{{ booking.totalAmount == null ? '待確認' : `NT$ ${formatMoney(booking.totalAmount)}` }}</strong></article>
          </div>
          <div v-else class="booking-empty compact"><ReceiptText :size="36" /><h3>這個月沒有服務明細</h3></div>
        </q-card-section>
      </q-card>
    </q-dialog>
    ...
    <q-dialog v-model="cancelDialog" persistent>
      <q-card class="cancel-dialog">
        <q-card-section class="detail-dialog__heading"><div><small>{{ bookingAction === 'CHANGE' ? '變更服務時間' : '取消服務' }}</small><h2>{{ bookingAction === 'CHANGE' ? '調整這筆預約' : '取消這筆任務' }}</h2></div><button class="detail-dialog__close" type="button" aria-label="關閉預約操作視窗" @click="cancelDialog = false"><X :size="24" /></button></q-card-section>
...
<q-dialog v-model="lineDialog">
  <q-card class="line-dialog">
    ...
    <q-card-section class="line-dialog__content">
      ...
      <p class="line-dialog__description">
        不熟悉網站操作也沒關係
<br>
        加入 LINE 後由專人協助您。
      </p>

      <div class="line-dialog__account">
        <span>LINE 官方帳號</span>
        <strong>@690hzupc</strong>
      </div>
...
const orderSummaryCardRef = ref<{ $el:HTMLElement } | null>(null);
const orderMonth = ref(taipeiDateKey(new Date()).slice(0, 7));
...
const displayedBookingDay = ref(selectedBookingDay.value);
const careScheduleDialogRef = ref<{ $el:HTMLElement } | null>(null);
const careCalendarRef = ref<HTMLElement | null>(null);
const selectedDayPanelRef = ref<HTMLElement | null>(null);
const nextSevenDaysRef = ref<HTMLElement | null>(null);
const sevenDayRailRef = ref<HTMLElement | null>(null);
let bookingDayMotionId = 0;
...
const selectedDayBookings = computed(() => bookings.value.filter((booking) => taipeiDateKey(booking.scheduledStartAt) === displayedBookingDay.value).sort((a,b) => +new Date(a.scheduledStartAt) - +new Date(b.scheduledStartAt)));
const canBookSelectedDay = computed(() => displayedBookingDay.value >= taipeiDateKey(new Date()));
const selectedBookingDateLabel = computed(() => taipeiDateTime(`${displayedBookingDay.value}T12:00:00+08:00`, { month:'long', day:'numeric', weekday:'long' }));
const selectedDayRelativeLabel = computed(() => displayedBookingDay.value === taipeiDateKey(new Date()) ? '今天' : canBookSelectedDay.value ? '未來行程' : '歷史紀錄');
...
const orderMonthBookings = computed(() => bookings.value
  .filter((booking) => taipeiDateKey(booking.scheduledStartAt).startsWith(orderMonth.value) && !['CANCELLED', 'ABANDONED'].includes(booking.status))
  .sort((a, b) => +new Date(a.scheduledStartAt) - +new Date(b.scheduledStartAt)));
const orderMonthTotal = computed(() => orderMonthBookings.value.reduce((total, booking) => total + (booking.totalAmount || 0), 0));
const orderPendingAmountCount = computed(() => orderMonthBookings.value.filter((booking) => booking.totalAmount == null).length);
const orderMonthLabel = computed(() => {
  const [year, month] = orderMonth.value.split('-');
...
const features: MemberFeature[] = [
  {
    title: '預約與照護', tone: 'coral', icon: markRaw(CalendarHeart), badge: '0 筆進行中',
    items: [
      { label: '查看預約及照護進度', caption: '目前沒有進行中的服務', icon: markRaw(CalendarClock) },
      { label: '訂單明細與總額', icon: markRaw(ReceiptText) },
  ...
  },
  {
    ...
    items: [
      ...
      { label: '新增家人資料',  icon: markRaw(HouseHeart) },
    ],
  },
  {
    title: '長照額度與費用', tone: 'sage', icon: markRaw(HandCoins), badge: '框架',
    items: [
      { label: '長照額度查詢', icon: markRaw(WalletCards) },
  ...
  },
  {
    ...
    items: [
      ...
      { label: 'LINE 帳號綁定', icon: markRaw(MessageCircleHeart) },
    ],
  },
  {
...
function selectBookingDay(date:string) {
  selectedBookingDay.value = date;
  void nextTick(() => selectedDayPanelRef.value?.scrollIntoView({ behavior:reducedBookingMotion() ? 'auto' : 'smooth', block:'nearest' }));
}
...
function reducedBookingMotion() { return window.matchMedia('(prefers-reduced-motion: reduce)').matches; }
function centerSelectedRailDay(animated = true) {
  const rail = sevenDayRailRef.value;
  const item = rail?.querySelector<HTMLElement>(`[data-booking-day="${selectedBookingDay.value}"]`);
  if (!rail || !item) return;
  const scrollLeft = Math.max(0, item.offsetLeft - (rail.clientWidth - item.offsetWidth) / 2);
  gsap.killTweensOf(rail);
  if (!animated || reducedBookingMotion()) { rail.scrollLeft = scrollLeft; return; }
  gsap.to(rail, { scrollLeft, duration:.38, ease:'power3.out', overwrite:true });
}
function animateSelectedDayItems() {
  const panel = selectedDayPanelRef.value;
  if (!panel || reducedBookingMotion()) return;
  const cards = panel.querySelectorAll('.care-booking-card');
  const empty = panel.querySelector('.selected-day-empty');
  gsap.killTweensOf([cards, empty]);
  if (cards.length) gsap.fromTo(cards, { y:16, opacity:0 }, { y:0, opacity:1, duration:.38, stagger:.07, ease:'power3.out', clearProps:'transform,opacity' });
  else if (empty) gsap.fromTo(empty, { y:14, opacity:0 }, { y:0, opacity:1, duration:.38, ease:'power3.out', clearProps:'transform,opacity' });
}
function animateBookingWorkspace() {
  const calendar = careCalendarRef.value;
  const panel = selectedDayPanelRef.value;
  const rail = nextSevenDaysRef.value;
  if (!calendar || !panel || !rail) return;
  gsap.killTweensOf([calendar, panel, rail]);
  if (reducedBookingMotion()) { gsap.set([calendar, panel, rail], { clearProps:'all' }); centerSelectedRailDay(false); return; }
  gsap.fromTo(calendar, { x:-30, opacity:0 }, { x:0, opacity:1, duration:.52, ease:'power3.out', clearProps:'transform,opacity' });
  gsap.fromTo(panel, { x:34, opacity:0 }, { x:0, opacity:1, duration:.52, ease:'power3.out', clearProps:'transform,opacity' });
  gsap.fromTo(rail, { y:18, opacity:0 }, { y:0, opacity:1, duration:.48, delay:.06, ease:'power3.out', clearProps:'transform,opacity' });
  centerSelectedRailDay(false);
}
function animateBookingDialogOpen() {
  displayedBookingDay.value = selectedBookingDay.value;
  const root = careScheduleDialogRef.value?.$el;
  if (!root) return;
  gsap.killTweensOf(root);
  if (reducedBookingMotion()) { gsap.set(root, { clearProps:'all' }); return; }
  gsap.fromTo(root, { y:28, opacity:0, scale:.985 }, { y:0, opacity:1, scale:1, duration:.52, ease:'power3.out', clearProps:'transform,opacity' });
  void nextTick(animateBookingWorkspace);
}
watch(bookingLoading, (loading) => { if (!loading && bookingDialog.value) void nextTick(animateBookingWorkspace); });
watch(selectedBookingDay, (day, previousDay) => {
  const motionId = ++bookingDayMotionId;
  void nextTick(() => centerSelectedRailDay());
  if (!bookingDialog.value || reducedBookingMotion()) { displayedBookingDay.value = day; void nextTick(animateSelectedDayItems); return; }
  const panel = selectedDayPanelRef.value;
  if (!panel) { displayedBookingDay.value = day; return; }
  const direction = day > previousDay ? 1 : -1;
  gsap.killTweensOf([panel, panel.querySelectorAll('.care-booking-card'), panel.querySelector('.selected-day-empty')]);
  gsap.to(panel, { x:-24 * direction, opacity:0, duration:.16, ease:'power3.out', overwrite:true, onComplete:() => {
    if (motionId !== bookingDayMotionId) return;
    displayedBookingDay.value = day;
    void nextTick(() => {
      if (motionId !== bookingDayMotionId) return;
      gsap.set(panel, { x:30 * direction, opacity:0 });
      gsap.to(panel, { x:0, opacity:1, duration:.34, ease:'power3.out', overwrite:true, clearProps:'transform,opacity', onComplete:animateSelectedDayItems });
    });
  }});
});
...
function canRescheduleBooking(booking:Booking) { return ['PENDING', 'ACCEPTED'].includes(booking.status); }
function canCancelBooking(booking:Booking) { return ['PENDING', 'ACCEPTED'].includes(booking.status); }
function selectManageableBooking(booking: Booking) { cancelBookingId.value = booking._id; }
function startReschedule(booking:Booking) { selectManageableBooking(booking); bookingAction.value = 'CHANGE'; changeDate.value = ''; changeSlot.value = ''; bookingDialog.value = false; cancelDialog.value = true; }
function prepareCancellation(booking: Booking) { selectManageableBooking(booking); bookingAction.value = 'CANCEL'; cancelReason.value = 'SCHEDULE_CHANGE'; bookingDialog.value = false; cancelDialog.value = true; }
...
async function openOrderSummary() { orderSummaryDialog.value = true; await loadBookings(); }
async function animateOrderSummaryOpen() {
  await nextTick();
  const root = orderSummaryCardRef.value?.$el;
  if (!root || reducedBookingMotion()) return;
  gsap.from(root.querySelectorAll('.order-total, .order-detail-card'), { y:12, opacity:0, duration:.32, stagger:.05, ease:'power3.out', clearProps:'transform,opacity' });
}
...
function handleFeatureItem(name: string) {
  ...
  if (name === '訂單明細與總額') { void openOrderSummary(); return; }
  if (name === '歷史照護日誌') {
    void openJournal();
    return;
  }
...
.overview-card { min-height:136px; display:flex; align-items:center; gap:16px; padding:22px;  background:
    rgba(255, 253, 251, .96);

  backdrop-filter: blur(4px);
  -webkit-backdrop-filter: blur(4px);border:1px solid rgb(110 87 80 / 12%); border-radius:22px; box-shadow:0 8px 24px rgba(73,56,51,.055); cursor:pointer; transition:transform .2s ease,box-shadow .2s ease,border-color .2s ease; touch-action:manipulation; }
...
.features-section { padding-top:62px; }.section-heading{display:flex;align-items:flex-end;justify-content:space-between;gap:24px;margin-bottom:26px}.section-heading span{color:var(--persimmon);font-size:.78rem;font-weight:800;letter-spacing:.12em}.section-heading h2 {
  margin: 7px 0 0;
...
}.section-heading p{margin:0;color:#7c655e;font-size:1rem}
.feature-grid {
  display: grid;
  ...
  line-height: 1.35;
}.feature-card header.coral {
  color: #a94a20;
  background: #fff0ea;
}
.feature-card header.wood {
  color: #5f4942;
  background: #f4ece7;
}.feature-card header.sage {
  color: #3f6254;
  background: #eaf2ed;
}
.feature-card header.peach {
  color: #a85643;
  background: #fff0ec;
}.feature-card header.amber {
  color: #9d5a2c;
  background: #fff3e8;
}.feature-card header.cream {
  color: #5e4a43;
  background: #f6efe9;
}
.feature-badge {
  ...
  color: var(--ink);
}.feature-list :deep(.q-item:hover) {
  background: #fff7f2;
}
.feature-list :deep(.q-item__section--avatar) {
  min-width: 36px;

  color: var(--persimmon);
}.feature-list :deep(.q-item__label) {
  color: var(--ink);
...
.feature-list :deep(.q-item__label--caption) {
  margin-top: 3px;
  ...
  gap: 7px;
}.feature-item-side :deep(.q-badge) {
  min-width: 22px;
...
.line-dialog__account span {
  font-size: .86rem;
}

.line-dialog__account strong {
  color: var(--ink);
...
.care-schedule-dialog{width:min(1120px,calc(100vw - 48px));max-width:min(1120px,calc(100vw - 48px))!important;height:min(820px,90dvh);display:flex;flex-direction:column;overflow:hidden;color:var(--ink);background:#fffdfb;border-radius:28px}.care-schedule-heading{flex:0 0 auto;padding-bottom:18px;border-bottom:1px solid #eadbd3}.care-schedule-heading p{margin:6px 0 0;color:#806a62;line-height:1.55}.care-schedule-scroll{flex:1 1 auto;min-height:0}.care-schedule-content{display:flex;flex-direction:column;gap:26px;padding:22px 30px 40px}.care-schedule-layout{display:grid;grid-template-columns:300px minmax(0,1fr);gap:22px;padding:0}.care-calendar-column{align-self:start;padding:12px;background:#fff7f2;border:1px solid #eadbd3;border-radius:22px}.care-mini-calendar{width:100%;min-width:0;background:transparent}.calendar-legend{display:flex;justify-content:center;flex-wrap:wrap;gap:8px 20px;padding:8px 4px 2px;color:#765f57;font-size:.82rem}.calendar-legend span{display:flex;align-items:center;gap:7px}.legend-dot{width:8px;height:8px;background:#777;border-radius:50%}.legend-dot.selected{background:#d65b20}.selected-day-panel{min-width:0;padding:20px;background:#fff;border:1px solid #eadbd3;border-radius:22px}.selected-day-heading{display:flex;align-items:flex-start;justify-content:space-between;gap:16px;padding-bottom:16px;border-bottom:1px solid #eee1da}.selected-day-heading small{color:#b84f16;font-weight:800}.selected-day-heading h3{margin:4px 0;font-size:1.45rem}.selected-day-heading p{margin:0;color:#806a62}.new-booking-btn{min-height:44px;padding:0 17px;color:#fff;background:#c55418;border-radius:13px;font-weight:800}.selected-booking-list{display:grid;gap:12px;margin-top:16px}.care-booking-card{display:grid;grid-template-columns:76px minmax(0,1fr);gap:16px;padding:16px;background:#fff9f5;border:1px solid #eddfd8;border-radius:18px}.care-booking-time{display:flex;flex-direction:column;gap:4px;color:#a44820}.care-booking-time strong{font-size:1.3rem}.care-booking-time small{color:#806a62}.care-booking-main{min-width:0}.care-booking-title{display:flex;align-items:start;justify-content:space-between;gap:12px}.care-booking-title h4{margin:0;font-size:1.08rem}.care-booking-title :deep(.q-badge){flex:none;padding:6px 9px;font-weight:700}.care-booking-meta{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:8px 14px;margin-top:12px;color:#6e5750;font-size:.88rem}.care-booking-meta span{display:flex;align-items:flex-start;gap:6px;min-width:0}.care-booking-meta svg{flex:none;color:#a44820}.care-booking-actions{display:flex;justify-content:flex-end;flex-wrap:wrap;gap:8px;margin-top:12px}.care-booking-actions :deep(.q-btn){min-height:44px;border-radius:12px;font-weight:800}.care-booking-actions .booking-action--danger{color:#963b30;background:#fff0ed}.selected-day-empty{min-height:220px;display:flex;flex-direction:column;align-items:center;justify-content:center;padding:24px;text-align:center;color:#9b5b3e}.selected-day-empty h4{margin:10px 0 5px;color:var(--ink);font-size:1.18rem}.selected-day-empty p{max-width:360px;margin:0 0 16px;color:#806a62}.next-seven-days{width:100%;padding:0;overflow:hidden}.section-divider{display:flex;align-items:center;gap:12px;margin:0 0 10px;color:#806a62;font-weight:800}.section-divider::before,.section-divider::after{height:1px;flex:1;background:#eadbd3;content:''}.seven-day-list{display:flex;gap:10px;overflow-x:auto;scroll-snap-type:x mandatory;overscroll-behavior-inline:contain;scrollbar-width:none}.seven-day-list::-webkit-scrollbar{display:none}.seven-day-row{width:145px;min-width:145px;min-height:116px;display:flex;align-items:flex-start;flex-direction:column;gap:6px;padding:11px;color:var(--ink);text-align:left;background:#fff;border:1px solid #eadfd9;border-radius:15px;cursor:pointer;scroll-snap-align:center}.seven-day-row:hover,.seven-day-row.is-selected{background:#fff3ec;border-color:#df9c7e}.seven-day-date{height:auto;display:flex;align-items:baseline;gap:5px;color:#a44820;background:transparent}.seven-day-date strong{font-size:1.2rem;line-height:1}.seven-day-date small{font-size:.72rem}.seven-day-copy{width:100%;min-width:0;display:grid;gap:1px}.seven-day-copy>span,.seven-day-copy small{overflow:hidden;color:#806a62;text-overflow:ellipsis;white-space:nowrap}.seven-day-copy small{font-size:.75rem}.seven-day-action{display:flex;align-items:center;gap:2px;margin-top:auto;color:#a44820;font-size:.8rem;font-weight:800}
...
@media(max-width:800px){.care-schedule-dialog{width:calc(100vw - 20px);max-width:calc(100vw - 20px)!important;height:94dvh;border-radius:22px}.care-schedule-heading{padding:18px 18px 12px}.care-schedule-heading h2{font-size:1.5rem}.care-schedule-heading p{display:none}.care-schedule-content{gap:18px;padding:14px 14px 28px}.care-schedule-layout{grid-template-columns:1fr;gap:12px}.care-calendar-column{width:100%;max-width:340px;justify-self:center;padding:8px}.selected-day-panel{min-height:260px;padding:15px}.selected-day-heading{align-items:stretch;flex-direction:column}.selected-day-heading :deep(.q-btn){width:100%}.care-booking-card{grid-template-columns:1fr}.care-booking-time{flex-direction:row;align-items:baseline}.care-booking-meta{grid-template-columns:1fr}.care-booking-actions{align-items:stretch;flex-direction:column}.seven-day-row{width:132px;min-width:132px;min-height:108px}}
.order-summary-dialog{width:min(720px,calc(100vw - 28px));max-width:min(720px,calc(100vw - 28px))!important;max-height:90dvh;display:flex;flex-direction:column;overflow:hidden;color:var(--ink);background:#fffdfb;border-radius:26px}.order-summary-body{min-height:0;display:grid;gap:16px;padding:4px 30px 28px;overflow-y:auto}.order-summary-body :deep(.q-field__control){border-radius:15px}.order-total{display:grid;gap:4px;padding:20px;color:#315f4c;background:#e8f3ed;border-radius:19px}.order-total>span{font-weight:800}.order-total>strong{font-size:2rem;font-variant-numeric:tabular-nums}.order-total>small{color:#587266}.order-detail-list{display:grid;gap:10px}.order-detail-card{display:flex;align-items:center;justify-content:space-between;gap:16px;padding:16px;background:#fff6f1;border:1px solid #eadfd9;border-radius:16px}.order-detail-card>div{min-width:0;display:grid;gap:3px}.order-detail-card span,.order-detail-card small{color:#806a62}.order-detail-card>strong{flex:none;font-variant-numeric:tabular-nums}.order-summary-dialog .detail-dialog__heading p{margin:5px 0 0;color:#806a62}@media(max-width:600px){.order-summary-body{padding:4px 16px 22px}.order-detail-card{align-items:flex-start;flex-direction:column;gap:8px}.order-detail-card>strong{align-self:flex-end}}
...
.order-month-field{min-height:56px;display:flex;align-items:center;justify-content:space-between;gap:16px;padding:8px 14px;border:1px solid #9d8c85;border-radius:15px}.order-month-field>span{color:#6f5b54}.order-month-field input{min-height:40px;padding:0 8px;color:var(--ink);background:transparent;border:0;font:inherit}.order-month-field:focus-within{outline:2px solid #c55418;outline-offset:1px}
</style>
