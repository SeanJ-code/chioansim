...
.ending{display:grid;grid-template-columns:45% 55%;align-items:center;gap:clamp(32px,5vw,72px);padding:clamp(80px,8vw,120px) max(24px,calc((100vw - 1440px)/2));background:linear-gradient(100deg,#edf1df,#fff9f5)}.ending-photo{order:2;overflow:hidden;aspect-ratio:4/3;border-radius:28px}.ending-photo img{width:100%;height:100%;object-fit:cover;filter:saturate(.86);will-change:transform}.ending-copy{order:1;will-change:transform,opacity}.ending-copy>svg{color:#4f7264}.ending-copy .eyebrow{margin-top:24px}.ending-copy button{min-height:48px;display:block;margin-top:18px;padding:0;color:var(--chestnut);background:transparent;border:0;text-decoration:underline;font:inherit;cursor:pointer}.cta-motion{display:inline-flex;will-change:transform,opacity}
...
@media(max-width:900px){.caregivers,.estimate,.ending{grid-template-columns:1fr}.caregiver-row{grid-auto-flow:column;grid-auto-columns:minmax(270px,78vw);grid-template-columns:none;overflow-x:auto;padding-bottom:16px;scroll-snap-type:x mandatory}.state{grid-column:auto}.trust-list{grid-column:auto}.booking-stage{grid-template-columns:repeat(2,1fr)}.booking-stage__path{display:none}.progress-steps{grid-template-columns:repeat(3,1fr)}.ending-photo,.ending-copy{order:initial}.ending-photo{max-height:460px}}
@media(max-width:599px){.caregivers,.booking-story,.progress-story,.estimate,.ending{padding:72px 18px}.caregiver-row{grid-auto-columns:82vw}.trust-list{grid-template-columns:1fr}.booking-stage{grid-template-columns:1fr;gap:20px;padding-left:24px}.booking-stage__path{left:8px;right:auto;top:8%;width:5px;height:84%;display:block;background:var(--persimmon)}.booking-path-line{display:none}.booking-node{min-height:145px}.progress-route{gap:12px;margin:42px 0 34px}.route-person svg,.route-home svg{width:38px}.progress-steps{grid-template-columns:1fr 1fr}.progress-steps li:last-child{grid-column:1/-1}.ending-copy{text-align:center}.ending-copy>svg{margin:auto}.ending-copy button{margin-inline:auto}.primary{width:100%;justify-content:center}}
@media(prefers-reduced-motion:reduce){.panel-reveal,.booking-path-line,.booking-node,.booking-heart,.route-line i,.cta-motion,.ending-copy,.ending-photo img{transform:none!important;will-change:auto!important}.route-dot{display:none}}
</style>
