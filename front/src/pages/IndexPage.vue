...
import { ArrowRight, BadgeCheck, BellRing, Bike, BriefcaseBusiness, CheckCircle2, ChevronRight, Heart, HouseHeart, MapPin, MapPinned, MessageCircleHeart, MessagesSquare, Route, ShieldCheck, ShoppingBasket, Stethoscope, UserRoundCheck, WifiOff, X } from '@lucide/vue';
...
const homePage = ref<HTMLElement>(), homeHero = ref<InstanceType<typeof HomeHero>>(), needs = ref<HTMLElement>(), journey = ref<HTMLElement>(), caregivers = ref<Caregiver[]>([]), caregiverLoading = ref(true), caregiverError = ref(false), lineDialog = ref(false);
...
function setupHomeScrollAnimations() {
  if (motionStarted || !homePage.value || !needs.value || !journey.value) return;
  motionStarted = true;
  const needsEl = needs.value, journeyEl = journey.value;
...
.home-page,.home-page main{padding:0;overflow:clip}.scene,.journey,.ending{width:100%;max-width:none;margin:0}.story-panel,.caregivers,.panel-reveal{border-radius:0}.needs,.caregivers,.progress-story,.booking,.estimate,.ending{min-height:100dvh;padding-inline:max(clamp(24px,5vw,96px),calc((100vw - 1440px)/2))}.needs,.caregivers,.progress-story,.booking,.estimate,.ending{margin:0}.journey{padding:0}.journey-stage{width:100%;min-height:100dvh;padding-inline:max(clamp(24px,5vw,96px),calc((100vw - 1440px)/2));border-radius:0}.estimate,.ending{padding-block:clamp(70px,9vw,110px)}
@media(max-width:900px){.home-page main{padding:0}.needs,.caregivers,.progress-story,.booking,.estimate,.ending,.journey-stage{padding-inline:clamp(22px,6vw,54px)}}
@media(max-width:599px){.home-page main{padding:0}.needs,.caregivers,.progress-story,.booking,.estimate,.ending,.journey-stage{min-height:auto;padding-inline:18px}.needs,.caregivers,.progress-story,.booking,.estimate,.ending{padding-block:72px}.journey-stage{padding-block:72px}.caregivers,.story-panel{border-radius:0}}
