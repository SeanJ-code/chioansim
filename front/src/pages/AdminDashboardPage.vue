<template>
  ...
  <q-page class="admin-page">
    ...
    <main ref="pageRef" class="admin-shell">
      ...
      <section ref="attentionSectionRef" class="attention-card" aria-labelledby="attention-title" data-admin-section>
      ...
      <q-tab-panels v-model="tab" animated class="admin-panels">
        ...
        <q-tab-panel name="overview">
          ...
          <div class="insight-grid overview-grid">
            ...
            <article ref="performanceSectionRef" class="insight-card ratings-card" data-admin-section>
        ...
        <q-tab-panel name="members">
          ...
          <div ref="memberSectionRef" data-admin-section>
          ...
          <section class="table-heading">
            ...
            <div class="member-tools">
              ...
              <q-input v-model="search" outlined dense clearable label="搜尋姓名、帳號、電話或信箱"><template #prepend><Search :size="20" /></template></q-input>

              <q-select v-model="memberStatus" outlined dense emit-value map-options label="帳號狀態" :options="memberStatusOptions" />
          ...
          <q-banner v-if="attentionFilter === 'DOCUMENT_ATTENTION'" rounded class="credential-filter-note">已篩選居服員；點選成員即可直接查看資格與待審文件。</q-banner>

          <q-table ref="memberTableRef" class="member-table" flat :rows="filteredUsers" :columns="memberColumns" row-key="_id" :loading="loading" :rows-per-page-options="[10, 20, 50]" no-data-label="目前沒有符合條件的成員">

            <template #body="props">

              <q-tr :props="props" class="member-table-row" :class="{ clickable: props.row.role === 'NURSE' }" @click="props.row.role === 'NURSE' && openCaregiver(props.row)">

                <q-td key="member" :props="props"><span class="member-name"><i>{{ props.row.name?.slice(0, 1) }}</i><b>{{ props.row.name }}</b><small>{{ props.row.account }}</small></span></q-td>

                <q-td key="role" :props="props"><q-badge outline :label="roleLabel(props.row.role)" /></q-td>

                <q-td key="contact" :props="props"><span class="contact"><b>{{ props.row.phone || '未填電話' }}</b><small>{{ props.row.email || '未填信箱' }}</small></span></q-td>

                <q-td key="status" :props="props"><q-badge :color="props.row.status === 'ACTIVE' ? 'positive' : 'grey-7'" :label="statusLabel(props.row.status)" /></q-td>

                <q-td key="actions" :props="props" @click.stop>
                  <q-btn flat round dense class="member-action-button" :aria-label="`管理 ${props.row.name}`">
                    <MoreHorizontal :size="20" aria-hidden="true" />
                    <q-menu anchor="bottom right" self="top right">
                      <q-list style="min-width: 180px">
                        <q-item v-if="props.row.role === 'NURSE'" v-close-popup clickable @click="openCaregiver(props.row)">
                          <q-item-section>查看居服員資料</q-item-section>
                        </q-item>
                        <q-item v-close-popup clickable @click="openEdit(props.row)">
                          <q-item-section>修改資料</q-item-section>
                        </q-item>
                        <q-separator />
                        <q-item v-close-popup clickable class="text-negative" @click="confirmHide(props.row)">
                          <q-item-section>隱藏成員</q-item-section>
                        </q-item>
                      </q-list>
                    </q-menu>
                  </q-btn>
        ...
        <q-tab-panel name="services">
          ...
          <div ref="bookingSectionRef" data-admin-section>

          <section class="table-heading">

            <div><p class="section-kicker">BOOKING PULSE</p><h2>最近預約與服務動向</h2></div>

            <div ref="bookingToolsRef" class="booking-tools" aria-label="篩選預約動向">
    ...
    <q-dialog v-model="safeReportDialog" transition-show="none" transition-hide="none">
<q-card
  v-if="selectedSafeReport"
  ref="adminSafeDialogRef"
  class="admin-safe-dialog"
  :class="`admin-safe-dialog--${normalizedSeverity(selectedSafeReport.priority).toLowerCase()}`"
>        <q-card-section class="admin-safe-dialog__header"><div><div class="safe-eyebrow"><span class="safe-priority-dot" /> SAFE REPORT · {{ severityLabel(selectedSafeReport.priority) }}</div><h2>{{ selectedSafeReport.reportNumber || '安全事件' }}</h2><p>{{ safeStatusCopy(selectedSafeReport.status) }}・{{ selectedSafeReport.complainantUserId?.name || '居服員' }}</p></div><q-btn flat round aria-label="關閉安全通報" @click="safeReportDialog = false"><X :size="21" /></q-btn></q-card-section>
        <q-separator />
        <q-card-section class="admin-safe-dialog__progress"><q-stepper :model-value="safeStep(selectedSafeReport.status)" flat alternative-labels color="deep-orange-8" class="safe-stepper"><q-step :name="1" title="已送出" prefix="1" :done="safeStep(selectedSafeReport.status)>1" done-icon="1" /><q-step :name="2" title="已確認" prefix="2" :done="safeStep(selectedSafeReport.status)>2" done-icon="2" /><q-step :name="3" title="處理追蹤" prefix="3" :done="safeStep(selectedSafeReport.status)>3" done-icon="3" /><q-step :name="4" title="已排除" prefix="4" :done="safeStep(selectedSafeReport.status)>4" done-icon="4" /><q-step :name="5" title="已結案" prefix="5" /></q-stepper></q-card-section>
        <q-separator />
        <q-card-section class="admin-safe-dialog__scroll"><div class="admin-safe-layout"><section><header class="safe-section-heading"><div><small>ORIGINAL REPORT</small><h3>原始通報</h3></div><span class="immutable-chip"><LockKeyhole :size="14" />不可修改</span></header><article class="safe-original-card"><div class="safe-original-card__meta"><span>{{ incidentLabel(selectedSafeReport.category) }}</span><time>{{ formatFullDate(selectedSafeReport.createdAt) }}</time></div><p>{{ selectedSafeReport.description }}</p></article><div v-if="selectedSafeReport.evidenceUrls?.length" class="safe-photo-grid"><img v-for="(photo, index) in selectedSafeReport.evidenceUrls" :key="photo" :src="safeAssetUrl(photo)" :alt="`安全通報原始照片 ${index + 1}`" /></div><header class="safe-section-heading safe-timeline-heading"><div><small>STATUS HISTORY</small><h3>處理時間軸</h3></div></header><q-timeline color="deep-orange-8" layout="dense"><q-timeline-entry v-for="activity in [...(selectedSafeReport.activities || [])].reverse()" :key="activity.createdAt" :title="activity.label" :subtitle="formatFullDate(activity.createdAt)" /></q-timeline></section><section class="safe-followup-column"><q-banner v-if="selectedSafeReport.status === 'SUBMITTED'" rounded class="acknowledge-banner"><template #avatar><BellRing :size="22" /></template><strong>等待您確認這件安全事件</strong><div>確認後會立即通知居服員，代表「已收到並介入」，不等同文字回覆。</div></q-banner><template v-else-if="selectedSafeReport.status !== 'CLOSED'"><header class="safe-section-heading"><div><small>FOLLOW UP</small><h3>新增處理紀錄</h3></div></header><q-input v-model="safeReply" outlined type="textarea" autogrow maxlength="2000" counter label="請輸入聯繫、處理或後續安排" /><div class="safe-composer-actions"><q-btn flat no-caps label="清除" :disable="!safeReply" @click="safeReply = ''" /><q-btn unelevated no-caps class="resolve-button" label="新增不可修改紀錄" :disable="!safeReply.trim()" :loading="safeSaving" @click="addSafeReply" /></div><div class="safe-status-actions"><q-btn v-if="selectedSafeReport.status === 'ACKNOWLEDGED'" outline no-caps label="開始處理追蹤" @click="advanceSafeReport('IN_PROGRESS')" /><q-btn v-if="selectedSafeReport.status === 'IN_PROGRESS'" outline no-caps color="positive" label="標記危險已排除" @click="advanceSafeReport('RESOLVED')" /><q-btn v-if="selectedSafeReport.status === 'RESOLVED'" outline no-caps color="grey-8" label="正式結案" @click="advanceSafeReport('CLOSED')" /></div></template><q-separator class="q-my-lg" /><header class="safe-section-heading"><div><small>ACTIVITY</small><h3>事件追蹤紀錄</h3></div><q-badge v-if="selectedSafeReport.replies?.length" rounded color="deep-orange-1" text-color="deep-orange-9" :label="`${selectedSafeReport.replies.length} 筆`" /></header><div ref="adminSafeAuditListRef" class="safe-audit-list"><article v-for="reply in sortedReplies(selectedSafeReport)" :key="reply._id || reply.createdAt" class="audit-reply"><header><strong>{{ reply.authorRole === 'ADMIN' ? '管理員' : '居服員' }}</strong><time>{{ formatFullDate(reply.createdAt) }}</time></header><p>{{ reply.message }}</p><small><LockKeyhole :size="13" /> 已送出・不可修改</small></article><div v-if="!selectedSafeReport.replies?.length" class="safe-empty-audit"><History :size="24" /><span>尚無追加紀錄</span></div></div></section></div></q-card-section>
        <q-separator v-if="selectedSafeReport.status === 'SUBMITTED'" />
        <q-card-actions v-if="selectedSafeReport.status === 'SUBMITTED'" align="right" class="admin-safe-dialog__actions"><q-btn flat no-caps label="關閉，稍後處理" @click="safeReportDialog = false" /><q-btn unelevated no-caps color="deep-orange-8" label="已確認並前往了解" :loading="safeSaving" @click="advanceSafeReport('ACKNOWLEDGED')" /></q-card-actions>
      </q-card>
    </q-dialog>
    ...
    <q-dialog v-model="editDialog">
      ...
      <q-card class="edit-dialog">
        ...
        <q-card-actions align="right"><q-btn flat no-caps label="先不修改" v-close-popup /><q-btn unelevated no-caps label="儲存調整" class="resolve-button" :loading="savingUser" @click="saveUser" /></q-card-actions>
...
const adminSafeDialogRef = ref<any>(null);
const adminSafeAuditListRef = ref<HTMLElement | null>(null);
...
const memberStatusOptions = [{ label: '全部狀態', value: 'ALL' }, ...statusOptions];
...
const attentionSectionRef = ref<HTMLElement | null>(null);

const performanceSectionRef = ref<HTMLElement | null>(null);

const bookingSectionRef = ref<HTMLElement | null>(null);

const bookingToolsRef = ref<HTMLElement | null>(null);

const memberSectionRef = ref<HTMLElement | null>(null);
...
const memberColumns: QTableColumn[] = [
  ...
  { name: 'member', label: '成員', field: 'name', align: 'left', sortable: true },

  { name: 'role', label: '身份', field: 'role', align: 'left', sortable: true },

  { name: 'contact', label: '聯絡方式', field: 'phone', align: 'left' },
...
const pulseCards = computed(() => [
  ...
  { label: '今日預約服務', value: dashboard.pulse.todayBookings || 0, note: `${dashboard.pulse.activeServices || 0} 件進行中`, icon: CalendarClock, tone: 'orange', action: 'BOOKINGS' as const },

  { label: '本月完成率', value: `${dashboard.performance.completionRate || 0}%`, note: `取消率 ${dashboard.performance.cancellationRate || 0}%`, icon: ChartNoAxesColumnIncreasing, tone: 'brown', action: 'OVERVIEW' as const },

  { label: '需要關心', value: attentionCount.value, note: `${dashboard.pulse.openComplaints || 0} 件申訴處理中`, icon: TriangleAlert, tone: 'red', action: 'CARE_ALERTS' as const },

  { label: '待審居服員文件', value: dashboard.pulse.pendingCredentials || 0, note: `共 ${dashboard.pulse.caregiverCount || 0} 位居服員`, icon: ShieldCheck, tone: 'green', action: 'CREDENTIALS' as const },
...
const filteredUsers = computed(() => {
  ...
  return users.value
    ...
    .filter((user) => memberStatus.value === 'ALL' || user.status === memberStatus.value)
...
const safeApiBaseUrl = import.meta.env.VITE_API_BASE_URL || '/api';
function safeAssetUrl(value: string) { return /^https?:\/\//.test(value) ? value : `${safeApiBaseUrl.replace(/\/api\/?$/, '')}${value.startsWith('/') ? value : `/${value}`}`; }
...
async function scrollToTarget(target: HTMLElement | null, highlight?: HTMLElement | null) {
  ...
  if (highlight) {
    gsap.fromTo(
      highlight,
      { y: 10, boxShadow: '0 0 0 3px rgb(217 107 39 / 35%)' },
      { y: 0, boxShadow: '0 0 0 0 rgb(217 107 39 / 0%)', duration: .45, clearProps: 'boxShadow' },
    );
  }

}

async function navigateAdminMetric(action: AdminMetricAction) {

  attentionFilter.value = '';

  if (action === 'BOOKINGS') { tab.value = 'services'; bookingDate.value = taipeiDateKey(new Date().toISOString()); bookingStatus.value = 'ALL'; }

  if (action === 'OVERVIEW') tab.value = 'overview';

  if (action === 'CREDENTIALS') { tab.value = 'members'; memberRole.value = 'NURSE'; memberStatus.value = 'ALL'; attentionFilter.value = 'DOCUMENT_ATTENTION'; }

  await nextTick();

  ScrollTrigger.refresh();

  if (action === 'BOOKINGS') await scrollToTarget(bookingSectionRef.value, bookingToolsRef.value);

  if (action === 'OVERVIEW') await scrollToTarget(performanceSectionRef.value, performanceSectionRef.value);

  if (action === 'CARE_ALERTS') await scrollToTarget(attentionSectionRef.value, attentionSectionRef.value);

  if (action === 'CREDENTIALS') await scrollToTarget(memberTableRef.value?.$el || memberSectionRef.value, memberTableRef.value?.$el);
...
async function handleAlert(alert: AlertItem, action: string, status: string) {
  ...
  try {
    ...
    if (status === 'RESOLVED') qualityDialog.value = false;
    await loadDashboard();
    liveSync.notifyChanged();
  } catch (error: any) {
    $q.notify({ type: 'negative', message: error?.response?.data?.message || '品質警訊處理失敗，請稍後再試。' });
  }
...
async function saveUser() {
  if (!editingId.value || savingUser.value) return;
  savingUser.value = true;
  try {
    await api.patch(`/admin/users/${editingId.value}`, editForm);
    editDialog.value = false;
    $q.notify({ type: 'positive', message: '成員資料已更新。' });
    await loadDashboard();
    liveSync.notifyChanged();
  } catch (error: any) {
    $q.notify({ type: 'negative', message: error?.response?.data?.message || '成員資料更新失敗，請稍後再試。' });
  } finally {
...
function confirmHide(user: PlainObject) {
  ...
  $q.dialog({ title: '確認隱藏這筆帳號？', message: `${user.name} 的資料不會真正刪除，日後仍可由稽核紀錄找回。`, cancel: { label: '先保留', flat: true }, ok: { label: '確認隱藏', color: 'negative', unelevated: true }, prompt: { model: '', type: 'text', label: '隱藏原因（選填）' } }).onOk(async (reason) => {
    try {
      await api.delete(`/admin/users/${user._id}`, { data: { reason } });
      $q.notify({ type: 'positive', message: '帳號已隱藏，原始關聯資料仍完整保留。' });
      await loadDashboard();
      liveSync.notifyChanged();
    } catch (error: any) {
      $q.notify({ type: 'negative', message: error?.response?.data?.message || '隱藏帳號失敗，請稍後再試。' });
    }
  });
...
onMounted(async () => {
  ...
  motionContext = gsap.context(() => {
    ...
    if (!window.matchMedia('(prefers-reduced-motion: reduce)').matches) {
      ...
      gsap.from('.pulse-card', { y: 18, autoAlpha: 0, duration: .4, stagger: .05, ease: 'power2.out' });

      gsap.from('.attention-item', { y: 12, autoAlpha: 0, duration: .35, stagger: .04, ease: 'power2.out', scrollTrigger: { trigger: attentionSectionRef.value, start: 'top 82%', once: true } });
...
onBeforeUnmount(() => { motionContext?.revert(); liveSync.stop(); });
...
.pulse-card:focus-visible { outline: 3px solid rgb(217 107 39 / 35%); outline-offset: 3px; }

.pulse-card > svg { flex: 0 0 auto; box-sizing: content-box; padding: 12px; border-radius: 15px; }.pulse-card div { display: grid; }
.pulse-card strong {
  color: var(--ink);
...
.pulse-card small {
  ...
  line-height: 1.45;
}.pulse-card.brown > svg {
  color: #6e5750;
  background: #f4ece7;
}.pulse-card.orange > svg {
  color: var(--persimmon);
  background: #fff0ea;
}.pulse-card.red > svg {
  color: #a94a43;
  background: #fff0ed;
}.pulse-card.green > svg {
  color: #4f7264;
  background: #edf3ef;
}
...
.attention-item { min-height: 72px; padding: 12px 24px; }.attention-item :deep(.q-item__section--side) { color: #755f57; }.attention-item :deep(.q-avatar) { width: 44px; height: 44px; }
.attention-high {
  color: #a7352c;
  background: #fff0ed;
}
...
.attention-low {
  color: #3f6254;
  background: #edf3ef;
}.attention-clear { display: flex; align-items: center; gap: 12px; padding: 18px 24px 24px; color: #3f725e; }
...
.admin-panels { background: transparent; }
.admin-panels :deep(.q-tab-panel) { padding: 0; }
...
}.insight-card header, .recent-reviews header { display: flex; align-items: center; gap: 13px; }
.quality-heading h2, .table-heading h2, .recent-reviews h2, .edit-dialog h2 { margin: 0; font-size: 1.35rem; }
.insight-card .section-kicker, .quality-heading .section-kicker, .table-heading .section-kicker, .recent-reviews .section-kicker, .edit-dialog .section-kicker { color: var(--persimmon); }
.insight-card h2,
.attention-card h2 {
  color: var(--ink);
...
.safe-report-banner__content{display:grid;gap:4px}
.safe-report-banner__content strong{color:#493833;font-size:.95rem}
.safe-report-banner__content span{margin:0;color:rgb(73 56 51/.65);font-size:.82rem}
.safe-report-banner__action {
  color: #9a4139;

  font-weight: 800;
}.safe-quick-filters{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:12px;margin:18px 0}
.safe-quick-filters button{min-height:64px;display:grid;grid-template-columns:auto 1fr auto;align-items:center;gap:10px;padding:12px 16px;color:#6e5750;text-align:left;background:#fffdfb;border:1px solid rgb(110 87 80/.14);border-radius:16px;cursor:pointer}.safe-quick-filters button:hover,.safe-quick-filters button:focus-visible{color:#a84513;border-color:rgb(200 86 24/.3);outline:none;box-shadow:0 8px 20px rgb(73 56 51/.07)}.safe-quick-filters button strong{min-width:28px;padding:4px 8px;text-align:center;background:#fff0e8;border-radius:999px}.safe-filter-bar{display:grid;grid-template-columns:auto minmax(180px,220px) minmax(200px,260px);align-items:end;gap:16px;margin:18px 0 20px}.safe-filter-group{display:grid;gap:6px}.safe-filter-label{color:rgb(73 56 51/.62);font-size:.75rem;font-weight:800}.safe-filter-bar .filter-toggle{margin:0}.safe-filter-select{background:#fffdfb}.safe-report-meta{margin-top:5px}.safe-report-description{max-width:680px;margin-top:3px}.safe-report-side{gap:6px}.safe-report-side small{color:#876f67}
...
@media(max-width:500px){.admin-safe-dialog__actions{display:grid;grid-template-columns:1fr 1.35fr}.admin-safe-dialog__actions .q-btn{min-height:48px}.safe-photo-grid{grid-template-columns:repeat(2,minmax(0,1fr))}.safe-original-card__meta{flex-direction:column;gap:5px}}
@media(max-width:850px){.admin-safe-dialog{height:calc(100dvh - 48px);max-height:calc(100dvh - 48px)}}
@media(prefers-reduced-motion:reduce){.admin-safe-dialog *{scroll-behavior:auto!important}}
</style>
