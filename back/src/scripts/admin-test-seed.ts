...
export async function seedAdminTestData(uri: string) {
  assert(uri && !/\/chioansim(?:\?|$)/.test(uri), '拒絕寫入主要資料庫；請使用獨立測試資料庫')
  ...
  const admin = await upsertUser('testadmin01', { name: '測試管理員', role: 'ADMIN', status: 'ACTIVE', email: 'test-admin-01@example.test' })
  const users = await Promise.all(Array.from({ length: 30 }, (_, i) => upsertUser(`testuser${String(i + 1).padStart(2, '0')}`, {
    ...
    email: `test-user-${i + 1}@example.test`,
  })))
  const nurses = await Promise.all(Array.from({ length: 10 }, (_, i) => upsertUser(`testcaregiver${String(i + 1).padStart(2, '0')}`, {
    name: `測試居服員${i + 1}`, role: 'NURSE', status: i === 9 ? 'PENDING' : 'ACTIVE',
    phone: `0911${String(i + 1).padStart(6, '0')}`, email: `test-caregiver-${i + 1}@example.test`,
  })))
  const caregivers = await Promise.all(nurses.map((user, i) => CaregiverProfile.findOneAndUpdate(
    { userId: user._id }, {
      userId: user._id, profilePhotoUrl: `https://api.dicebear.com/9.x/notionists/svg?seed=test-caregiver-${i + 1}`,
      introduction: `測試居服員 ${i + 1}，提供虛構測試服務。`, yearsExperience: i + 1,
      serviceAreas: ['台北市', i % 2 ? '新北市' : ''], certificateNumber: `TEST-CERT-${String(i + 1).padStart(2, '0')}`,
  ...
  const service = await ServiceType.findOneAndUpdate({ code: 'TEST-DAILY' }, {
    code: 'TEST-DAILY', name: '測試生活照護', description: 'Admin 整合測試專用', durationMinutes: 120, basePrice: 900, active: true,
  }, { upsert: true, new: true })

  const recipients = await Promise.all(Array.from({ length: 20 }, (_, i) => CareRecipient.findOneAndUpdate(
    { name: `test-recipient-${String(i + 1).padStart(2, '0')}` }, {
      createdByUserId: users[i % users.length]._id, name: `test-recipient-${String(i + 1).padStart(2, '0')}`,
      ...
      carePhotoUrls: i % 5 ? [`https://api.dicebear.com/9.x/notionists/svg?seed=test-recipient-${i + 1}`] : [],
  ...
  for (let i = 0; i < 45; i += 1) {
    ...
    const request = await ServiceRequest.findOneAndUpdate({ specialRequirements: `test-booking-${String(i + 1).padStart(3, '0')}` }, {
      ...
      serviceAddress: recipients[i % recipients.length].address, specialRequirements: `test-booking-${String(i + 1).padStart(3, '0')}`,
    ...
    const booking = await Booking.findOneAndUpdate({ bookingNumber: `test-booking-${String(i + 1).padStart(3, '0')}` }, {
      bookingNumber: `test-booking-${String(i + 1).padStart(3, '0')}`, serviceRequestId: request._id,
      requesterUserId: users[i % users.length]._id, recipientId: recipients[i % recipients.length]._id,
  ...
  await Promise.all(caregivers.map((caregiver, i) => CaregiverCredential.findOneAndUpdate({ caregiverId: caregiver._id, number: `TEST-SKILL-${i + 1}` }, {
    caregiverId: caregiver._id, kind: 'SKILL', name: '測試照護技能', number: `TEST-SKILL-${i + 1}`, fileUrl: '/seed-assets/caregivers/test.pdf', verificationStatus: i === 9 ? 'PENDING' : 'APPROVED', expiresAt: days(i === 8 ? 15 : 400),
  }, { upsert: true })))
  await Promise.all(bookings.slice(0, 12).map((booking, i) => Notification.findOneAndUpdate({ bookingId: booking._id, title: `測試通知 ${i + 1}` }, {
    recipientUserId: booking.requesterUserId, type: 'BOOKING', title: `測試通知 ${i + 1}`, message: i % 3 ? '預約狀態已有更新。' : '請確認本次服務安排。', bookingId: booking._id, status: i % 4 ? 'SENT' : 'READ', readAt: i % 4 ? undefined : new Date(),
  }, { upsert: true })))
  await Promise.all(bookings.slice(0, 5).map((booking, i) => Complaint.findOneAndUpdate({ bookingId: booking._id, description: `test-complaint-${i + 1}` }, {
    complainantUserId: booking.requesterUserId, targetUserId: nurses[i]._id, bookingId: booking._id, category: i % 2 ? '遲到' : '溝通', description: `test-complaint-${i + 1}`, priority: i === 0 ? 'HIGH' : 'NORMAL', status: i < 3 ? 'SUBMITTED' : 'RESOLVED', assignedAdminId: i < 3 ? undefined : admin._id,
  }, { upsert: true })))
  ...
  const counts = {
    users: await User.countDocuments({ account: /^testuser/ }), recipients: await CareRecipient.countDocuments({ name: /^test-recipient-/ }),
    caregivers: await User.countDocuments({ account: /^testcaregiver/ }), bookings: await Booking.countDocuments({ bookingNumber: /^test-booking-/ }),
    reviews: await Review.countDocuments({ bookingId: { $in: bookings.map((b) => b._id) } }), notifications: await Notification.countDocuments({ title: /^測試通知 / }),
  }
...
if (require.main === module) seedAdminTestData(process.env.ADMIN_TEST_MONGODB_URI || '').then((result) => { console.log(result); return mongoose.disconnect() }).catch((error) => { console.error(error); process.exitCode = 1 })
