...
async function main() {
  ...
  try {
    assert.equal((await request('/admin/safe-reports/[object%20Object]/status', token, 'PATCH', { status: 'ACKNOWLEDGED' })).status, 400)
    assert.equal((await request(`/admin/safe-reports/${new Types.ObjectId()}/status`, token, 'PATCH', { status: 'ACKNOWLEDGED' })).status, 404)

    for (const status of ['ACKNOWLEDGED', 'IN_PROGRESS', 'RESOLVED', 'CLOSED']) {
      const result = await request(`/admin/safe-reports/${reportId}/status`, token, 'PATCH', { status })
    ...
    assert.equal((await request(`/admin/safe-reports/${reportId}/status`, token, 'PATCH', { status: 'CLOSED' })).status, 409)
    const reply = await request(`/feedback/complaints/${reportId}/replies`, token, 'POST', { message: '不可修改的流程驗證紀錄' })
