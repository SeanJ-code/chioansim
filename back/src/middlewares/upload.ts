...
export const validateJournalImages: RequestHandler = async (request, _response, next) => {
  ...
  try {
    for (const file of files) {
      ...
      await handle.close()
      if (!hasImageSignature(buffer)) throw new Error('服務照片內容不是有效的 JPG、PNG 或 WebP 圖片。')
    }
    next()
  } catch (error) {
    await Promise.all(files.map((file) => fs.promises.unlink(file.path).catch(() => undefined)))
    next(Object.assign(error instanceof Error ? error : new Error('服務照片驗證失敗'), { statusCode: 400 }))
  }
}
