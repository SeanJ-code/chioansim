...
function toCenters(text: string): LtcCenter[] {
  ...
  return rows.flatMap((row) => {
    const lat = Number(row[column('緯度')]), lng = Number(row[column('經度')])
...
async function getCenters(): Promise<LtcCenter[]> {
  ...
  try {
    const response = await fetch(DATA_URL, { signal: AbortSignal.timeout(15_000) })
    if (!response.ok) throw new Error(`LTC data responded ${response.status}`)
    const centers = toCenters(await response.text())
    if (!centers.length) throw new Error('LTC data was empty')
...
ltcRoutes.get('/nearby', asyncHandler(async (request, response) => {
  const lat = Number(request.query.lat), lng = Number(request.query.lng)
  ...
  const nearby = (await getCenters())
    .map((center) => ({ ...center, distanceMeters: Math.round(distanceMeters(lat, lng, center.lat, center.lng)) }))
